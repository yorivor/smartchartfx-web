<template>
  ...
  >
    <template>
      <v-card>
        <v-toolbar color="primary">Edit Pruchase Order</v-toolbar>
        <v-container>
...
export default {
  ...
  methods: {
    ...
    submit() {
      let url = this.$api + "/preparer/purchase-orders/" + this.form.id;
      this.isLoading = true;
  ...
  watch: {
    show: function () {
      ...
      if (!this.show) {
        ...
        this.form.company_address = this.item.company_address_id;
        this.form.company_contact = this.item.company_contact_id;
        this.form.vendor = this.item.vendor_id;
